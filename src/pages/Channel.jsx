  ...
  useEffect(() => {
    const fetchResulits = async () => {
      try {
        ...
        const videsData = await fetchFromAPI(
          `search?channelId=${channelId}&part=snippet%2Cid&order=date`
        );
        setChannelVideo(videsData?.items);
        setNextPageToken(videsData?.nextPageToken);
      } catch (error) {
      ...
      <section id="channel" className={channelPageClass}>
        {channelDetail && (
          <div className="channel__inner">
            ...
            <div className="channel__videio video__inner search">
